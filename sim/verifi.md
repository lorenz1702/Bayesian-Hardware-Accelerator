...
- [x] **TC-1.1 Single Transaction**
  ...
  - **Expected Behavior:** After a few cycles of latency (CLT noise generation + ALU computation), `bnn_valid` must assert for exactly one clock cycle. The FSM must assert `ready` exactly once to consume the input. The result must match the Golden Model.
- [x] **TC-1.2 Continuous Flow**
  ...
  - **Expected Behavior:** The system processes data continuously. `bnn_valid` must stream results out, and the FSM must properly toggle `ready` to throttle the input according to the ALU's computation time. No data loss is allowed.
...
- [x] **TC-2.1 Receiver Backpressure**
  ...
  - **Expected Behavior:** The FSM must freeze in the `WAIT_FOR_ALU` state once a result is ready. Crucially, it must keep `ready = 0` so the testbench cannot send any new `x_in` data until the current output is finally read.
...
- [x] **TC-3.2 CLT Delay Tolerance**
  ...
  - **Expected Behavior:** The FSM must wait in the `WAIT_FOR_CLT` state. It must not assert `ready` to consume the input, nor start the ALU, until the noise (`epsilon`) is completely ready.
...
3. **Stimulus Driver:** Pushes data into the top-level module based on the `x_valid` & `ready` handshake protocol.
